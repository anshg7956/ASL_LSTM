...
def fit_to_frame(keypoints_list, frame_size, margin=50):
    w, h = frame_size
    
    # Collect all coords (pose only, for stability)
    all_x, all_y = [], []
    for frame in keypoints_list:
        if frame['pose'].size > 0:
            all_x.extend(frame['pose'][:, 0])
            all_y.extend(frame['pose'][:, 1])
    
    xmin, xmax = min(all_x), max(all_x)
    ymin, ymax = min(all_y), max(all_y)
    
    # Compute scale while preserving aspect ratio
    scale_x = (w - 2*margin) / (xmax - xmin)
    scale_y = (h - 2*margin) / (ymax - ymin)
    ...
    for frame in keypoints_list:
        for part in ['pose','face','hand_left','hand_right']:
            arr = frame[part]
            if arr.size > 0:
                arr[:, 0] = (arr[:, 0] - x_center) * scale + w // 2
                arr[:, 1] = (arr[:, 1] - y_center) * scale + h // 2
    
    return keypoints_list


def load_keypoints_from_json_list(json_file, frame_size=(1280, 720)):
    """
    Load keypoints from a single JSON file containing a list of frame data.
    Expected format: A JSON array where each element is what was previously in individual files
    [
    ...
    """
    keypoints_by_frame = []
    w, h = frame_size  # <-- use output video size for scaling
    
    with open(json_file, 'r') as f:
        frames_data = json.load(f)
    
    print(f"Loading {len(frames_data)} frames from {json_file}")
    ...
    for frame_idx, frame_data in enumerate(frames_data):
        if not frame_data.get('people') or len(frame_data['people']) == 0:
            ...
            def reshape_and_scale(keypoints, expected):
                if keypoints:
                    arr = np.array(keypoints).reshape((-1, 3))  # keep triplets
                    arr[:, 0] *= w   # scale x
                    arr[:, 1] *= h   # scale y
                    arr[:, 2] = 1.0  # overwrite confidence with 1.0
                    return arr
            ...
            frame_keypoints = {
                'pose': reshape_and_scale(person.get('pose_keypoints_2d', []), 25),
                'face': reshape_and_scale(person.get('face_keypoints_2d', []), 70),
                'hand_left': reshape_and_scale(person.get('hand_left_keypoints_2d', []), 21),
                'hand_right': reshape_and_scale(person.get('hand_right_keypoints_2d', []), 21),
            }
        
        keypoints_by_frame.append(frame_keypoints)
    
    return keypoints_by_frame
def load_keypoints_from_json_list_no_conf(json_file, frame_size=(1280, 720)):
    """
    Load keypoints from JSON where each keypoint is just x,y (no confidence).
    Adds confidence=1.0 internally.
    """
    keypoints_by_frame = []
    w, h = frame_size
    ...
    for frame_data in frames_data:
        if not frame_data.get('people') or len(frame_data['people']) == 0:
            ...
            def reshape_xy(keypoints, expected):
                if keypoints:
                    arr = np.array(keypoints).reshape((-1, 2))
                    arr[:, 0] *= w  # scale x
                    arr[:, 1] *= h  # scale y
                    confidence = np.ones((arr.shape[0], 1))
                    return np.hstack([arr, confidence])  # add dummy confidence
                return np.zeros((expected, 3))

            frame_keypoints = {
                'pose': reshape_xy(person.get('pose_keypoints_2d', []), 25),
                'face': reshape_xy(person.get('face_keypoints_2d', []), 70),
                'hand_left': reshape_xy(person.get('hand_left_keypoints_2d', []), 21),
                'hand_right': reshape_xy(person.get('hand_right_keypoints_2d', []), 21),
            }
...
def load_keypoints_from_json_sequence(json_folder):
    """
    Legacy function for loading from multiple JSON files in a folder.
...
def draw_keypoints(frame, frame_data, confidence_threshold=0.3):
    ...
    """
    Draw only pose keypoints on a frame.
    
    Args:
        frame: OpenCV image (BGR format)
        frame_data: Dict containing 'pose' keypoints
        confidence_threshold: minimum confidence to draw keypoint
    """
    h, w = frame.shape[:2]
    ...
    # Scale keypoints if they are normalized (0-1)
    scaled_keypoints = []
    for kp in pose_keypoints:
        x, y, c = kp
        if x <= 1.0 and y <= 1.0:  # assume normalized
            x = int(x * w)
            y = int(y * h)
        else:  # already in pixels
            x = int(x)
            y = int(y)
        scaled_keypoints.append((x, y, c))
    
    # Draw skeleton
    for kp1_idx, kp2_idx in POSE_CONNECTIONS:
        if (kp1_idx < len(scaled_keypoints) and kp2_idx < len(scaled_keypoints) and
            scaled_keypoints[kp1_idx][2] > confidence_threshold and
            scaled_keypoints[kp2_idx][2] > confidence_threshold):
            
            pt1 = (scaled_keypoints[kp1_idx][0], scaled_keypoints[kp1_idx][1])
            pt2 = (scaled_keypoints[kp2_idx][0], scaled_keypoints[kp2_idx][1])
            
            # Color skeleton connections
            if kp1_idx in [2, 3, 4] or kp2_idx in [2, 3, 4]:  # Right arm
                color = (0, 255, 255)
            elif kp1_idx in [5, 6, 7] or kp2_idx in [5, 6, 7]:  # Left arm
                color = (0, 128, 255)
    ...
    # Draw keypoints
    for i, kp in enumerate(scaled_keypoints):
        x, y, c = kp
...
if __name__ == "__main__":
    frame_size = (1280, 720)
    keypoints_list = load_keypoints_from_json_list_no_conf(
        'output_hello_model/output_hello_v2.json',
        frame_size=frame_size
    )

    # Auto-fit skeleton to frame
    keypoints_list = fit_to_frame(keypoints_list, frame_size, margin=100)

    create_keypoints_video(
        keypoints_list=keypoints_list,
        output_path='video_file_output_hello_v2GEN/pose_visualization_coordinates_v0.mp4',
        fps=24,
        frame_size=frame_size,
